...
// TODAY TAB
// ═══════════════════════════════════════════
var DEFAULT_BLOCK_ORDER = ['affirmation','calendar','why','intimacy','mood','flow','fertility','fluids','sleep','symptoms','relief','tasks','note','ritual'];
...
function renderWeekPlanCard() {
  ...
  for (var i = 0; i < 7; i++) {
    ...
    html += '<div class="week-plan-row" style="align-items:flex-start;gap:8px;flex-wrap:wrap;">';
    html += '<span>' + (i===0?'today':formatCycleRangeDate(ds)) + '</span><strong>' + ph.moon + ' day ' + cday + '</strong><em>' + tip + '</em>';
    html += '<div style="width:100%;display:flex;gap:6px;margin-top:6px;">';
    html += '<input id="week-plan-input-' + ds + '" placeholder="plan a task for this day..." style="flex:1;border:1px solid var(--bg2);border-radius:18px;padding:8px 10px;background:var(--bg);color:var(--text);font-size:.74rem;">';
    html += '<button class="btn-sm" onclick="addWeekPlanItem(\'' + ds + '\')" style="padding:7px 10px;font-size:.68rem;">add</button>';
    html += '</div>';
    if (suggestions.length) {
      html += '<div style="width:100%;display:flex;gap:5px;flex-wrap:wrap;margin-top:3px;">';
    ...
    if (heavy) html += '<div class="flag-row" style="width:100%;margin-top:6px;">load check: this may be a lot for luteal. Consider moving one item earlier or softening it.</div>';
    html += '</div>';
  }
  html += '</div></div>';
  return html;
