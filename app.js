...
function renderCalendarBlock() {
  ...
  var predictedStarts = [];
  if (!appState.profile.pregnant && sortedStarts.length) {
    var predictionLimit = new Date(todayDate);
    predictionLimit.setMonth(predictionLimit.getMonth() + 3);
    var nextPredictedStart = new Date(sortedStarts[sortedStarts.length-1] + 'T00:00:00');
    nextPredictedStart.setDate(nextPredictedStart.getDate() + avgCycleLen);
    while (nextPredictedStart <= predictionLimit) {
      predictedStarts.push(new Date(nextPredictedStart));
      nextPredictedStart.setDate(nextPredictedStart.getDate() + avgCycleLen);
    }
  }
  ...
  for (var d = 1; d <= last.getDate(); d++) {
    ...
    // Mark predicted periods and ovulation/fertile windows up to three months ahead.
    var isPredicted = false;
    predictedStarts.forEach(function(pstart) {
      var predictedDiff = Math.floor((thisTime - pstart.getTime()) / 86400000);
      if (predictedDiff >= 0 && predictedDiff < (appState.profile.period_len || 5) && !isPeriod) isPredicted = true;
      var predictedOvDay = Math.round(avgCycleLen * 0.5);
      if (!isPeriod && !isPredicted) {
        if (predictedDiff >= (predictedOvDay - 6) && predictedDiff < predictedOvDay) isFertile = true;
        if (predictedDiff === predictedOvDay) isOvulation = true;
      }
    });
...
function renderCycleComparisonCard() {
  ...
  function diffLine(label, now, then, suffix) {
    ...
    return '<div style="display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:12px;padding:8px 16px;border-bottom:1px solid var(--bg2);">'
      + '<span style="font-size:clamp(.8rem,3.1vw,.88rem);color:var(--text);line-height:1.35;min-width:0;">' + label + '</span>'
      + '<strong style="font-size:clamp(.78rem,3vw,.86rem);color:var(--text);font-weight:500;text-align:right;white-space:nowrap;">' + text + '</strong></div>';
  }
...
function renderPredictionCards() {
  ...
  if (!pmsData.inLuteal && pmsData.daysUntilLuteal !== undefined) {
    ...
    html += '<div style="background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);border-left:3px solid #6a5a9a;padding:14px 16px;margin-bottom:14px;display:flex;align-items:center;gap:12px;">';
    html += '<span style="font-size:1.4rem;">🌗</span><div>';
    html += '<div style="font-size:clamp(.6rem,2.3vw,.66rem);letter-spacing:.12em;text-transform:uppercase;color:var(--brand);margin-bottom:2px;">luteal day ' + pmsData.lutealDay + '</div>';
    html += '<div style="font-size:clamp(.84rem,3.3vw,.92rem);color:var(--text);">' + periodTiming + '</div>';
    if (dtp <= (appState.profile.pms_notice_days || 3)) html += '<div style="font-size:clamp(.72rem,2.8vw,.78rem);color:var(--text2);margin-top:3px;">&#128138; magnesium + early bedtime will help</div>';
    html += '</div></div>';
  }
  ...
  if (ovData.status === 'upcoming') {
    ...
    html += '<div style="background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);border-left:3px solid #5a8a6a;padding:14px 16px;margin-bottom:14px;display:flex;align-items:center;gap:12px;">';
    html += '<span style="font-size:1.4rem;">🌸</span><div>';
    ...
    html += '<div style="background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);border-left:3px solid #b8832a;padding:14px 16px;margin-bottom:14px;display:flex;align-items:center;gap:12px;">';
    html += '<span style="font-size:1.4rem;">🌕</span><div>';
  ...
  if (appState.profile.fertility_tracking !== false) {
    if (fertilitySummary.likelyDate) {
      html += '<div style="background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);border-left:3px solid #5a8a6a;padding:14px 16px;margin-bottom:14px;display:flex;align-items:flex-start;gap:12px;">';
