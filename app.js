...
function renderAllTabs() {
  var fns = [renderToday, renderSupport, renderLog, renderInsights, renderHabits, renderLibrary];
...
// TODAY TAB
// ═══════════════════════════════════════════
var DEFAULT_BLOCK_ORDER = ['affirmation','calendar','why','ritual','intimacy','mood','flow','fertility','fluids','sleep','symptoms','relief','tasks','note'];
...
function repairHiddenBlocks() {
  ...
  // Also ensure core blocks (mood, tasks, flow) are never silently hidden
  var core = ['mood','tasks','flow'];
...
function renderToday() {
  ...
  if (isClosingRitualDay(cycleDay, cycleLen)) {
...
function renderBlock(blockId, log, phase, cycleDay) {
  ...
  if (blockId === 'tasks') return renderTasksBlock(log, phase);
...
function renderFirstWeekGuide() {
  ...
  steps.forEach(function(step) {
    ...
    html += '<span style="' + (done ? 'text-decoration:line-through;color:var(--text3);' : '') + '">' + step.text + '</span>';
    html += '</div></button>';
  });
  html += '</div>';
  return html;
...
function renderTasksBlock(log, phase) {
  ...
  if (phaseSuggestions.length) {
    ...
    phaseSuggestions.slice(0,8).forEach(function(s) {
      ...
      html += '<span class="chip chip-sm" onclick="saveSuggestionForLater(\'' + s.replace(/'/g,"\\'") + '\',\'' + phase.name + '\')" style="margin:2px;background:transparent;color:var(--text3);border:1px dashed var(--bg2);">save</span>';
    });
    html += '</div></div>';
  }
...
function toggleSymptom(name) {
  ...
  if (idx > -1) log.symptoms.splice(idx,1); else log.symptoms.push(name);
  saveTodayLog();
...
function renderLog() {
  ...
  if (logSyms.length) {
    ...
    logSyms.forEach(function(s){ html += '<span style="padding:4px 10px;border-radius:20px;background:var(--bg2);font-size:clamp(.72rem,2.8vw,.78rem);color:var(--text2);">'+s+'</span>'; });
    html += '</div></div></div>';
  }
...
function renderLoggedDaySummary(dateStr, log, phase, cycleDay) {
  ...
  if (syms.length) {
    cards += '<div class="card" style="margin-bottom:12px;"><div class="card-header"><span class="card-title">symptoms</span></div><div class="card-body">' + chipList(syms) + '</div></div>';
  }
...
function renderWeekPlanCard() {
  var html = '<div class="card"><div style="padding:16px;">';
  html += '<div style="font-size:clamp(.6rem,2.3vw,.66rem);letter-spacing:.13em;text-transform:uppercase;color:var(--brand);margin-bottom:8px;">plan the next 7 days</div>';
  for (var i = 0; i < 7; i++) {
    ...
    html += '<div class="week-plan-row"><span>' + (i===0?'today':formatCycleRangeDate(ds)) + '</span><strong>' + ph.moon + ' day ' + cday + '</strong><em>' + tip + '</em></div>';
  }
  html += '</div></div>';
  return html;
}
...
function getAdvancedPatternInsights() {
  ...
  Object.keys(appState.dayLogs || {}).forEach(function(d) {
    ...
    var syms = []; try { syms = typeof log.symptoms === 'string' ? JSON.parse(log.symptoms) : (log.symptoms || []); } catch(e) {}
    var moods = []; try { moods = typeof log.mood_words === 'string' ? JSON.parse(log.mood_words) : (log.mood_words || []); } catch(e) {}
    syms.concat(moods).forEach(function(item) { rows.push({ item:item, cday:cday, daysToPeriod:daysToPeriod, phase:getPhase(cday, getPredictionCycleLength()).name }); });
  });
  var grouped = {};
...
function renderSettings() {
  ...
  html += '<div class="settings-section-title">app basics</div>';
  ...
  html += '<div style="font-size:clamp(.68rem,2.6vw,.74rem);color:var(--text3);margin-top:8px;line-height:1.5;">A quick reference for what to log, when to trust insights, and how to protect local data.</div>';
  html += '</div>';
  ...
  html += '<div class="settings-section">';
  html += '<div class="settings-section-title">mood emojis</div>';
  ...
  html += '<div class="settings-section">';
  html += '<div class="settings-section-title">mood words</div>';
  ...
  // Custom symptoms
  html += '<div class="settings-section">';
  html += '<div class="settings-section-title">custom symptoms</div>';
  ...
  html += '<div class="settings-section">';
  html += '<div class="settings-section-title">work suggestions per phase</div>';
  ...
  // Affirmations
  html += '<div class="settings-section">';
  html += '<div class="settings-section-title">affirmations per phase</div>';
  html += '<div class="settings-phase-tabs">';
  ['menstrual','follicular','ovulatory','luteal'].forEach(function(p) {
    html += '<span class="settings-phase-tab' + (settingsPhaseTab===p?' active':'') + '" onclick="setSettingsPhase(\'' + p + '\')">' + p + '</span>';
  });
...
function renderHabits() {
  ...
  // View toggle
  html += '<div style="display:flex;gap:6px;">';
  ['today','month','year'].forEach(function(v){
    var act = view === v;
...
function showReportCard() {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:500;background:rgba(0,0,0,0.85);display:flex;flex-direction:column;align-items:center;justify-content:flex-start;overflow-y:auto;padding:20px;';

  var img = document.createElement('img');
  img.src = canvas.toDataURL('image/png');
  img.style.cssText = 'width:100%;max-width:420px;border-radius:16px;margin-top:10px;';

  var btnRow = document.createElement('div');
  btnRow.style.cssText = 'display:flex;gap:10px;margin-top:16px;width:100%;max-width:420px;';
  ...
  closeBtn.style.cssText = 'position:fixed;top:20px;right:20px;background:rgba(255,255,255,0.15);border:none;color:#fff;width:36px;height:36px;border-radius:50%;font-size:1rem;cursor:pointer;z-index:501;';
...
function showMaximizeGuide() {
  ...
  [
    ['today tab','Your daily home base. Use it for the quick check-in: affirmation, phase context, mood, flow, fertility signs if enabled, water, sleep, symptoms, relief ideas, tasks, notes, and rituals.'],
    ['log tab','Your calendar-style body record. Today stays editable, while past dates show a clean snapshot of only what was actually recorded. This is also where cycle starts and past cycles live.'],
    ['habits tab','Track the routines that support your body. Use Today, Month, and Year views to see consistency without perfection. Habit goals can be daily, weekdays, 3x/week, or luteal-only.'],
    ['insights tab','This is where the app reads the data back to you: prediction confidence, cycle recap, pattern reads, cycle-to-cycle comparison, PMS care plans, cycle health notes, and exportable reports.'],
    ...
    ['settings','Personalize the app name, greeting, cycle details, fertility tracker, pregnancy mode, dark mode, custom moods, custom symptoms, custom focus tasks, and affirmations.'],
  ...
  html += '<div style="font-size:clamp(.76rem,3vw,.82rem);color:var(--text2);line-height:1.8;">daily: tiny check-in<br>weekly: glance at habits + upcoming phase<br>monthly: review insights + export backup</div></div>';
  html += '</div>';
  overlay.innerHTML = html;
  document.body.appendChild(overlay);
}
...
// Prevent body scroll/drag on iOS — only allow scrolling inside #tab-content
document.body.addEventListener('touchmove', function(e) {
  var tc = document.getElementById('tab-content');
  if (tc && tc.contains(e.target)) return; // allow scroll in content area
  e.preventDefault();
}, { passive: false });
